...
export async function getFullRecipeInfo(recipeId) {
  try{
    return await databases.getDocument(
      databaseId,
      recipeCollectionId,
      recipeId,
      [Query.select(['$id','title', 'description', 'image', 'ingredients', 'steps'])]
    )
  }
  catch(error){
    throw new Error(error)
...
export async function addRecipe(recipe) {
  try {
    recipe.image = await uploadFile(recipe.image)
...
export async function changeRecipe(recipe, imageToDeleteURL) {
  try {
    if (typeof recipe.image !== 'string') {
      recipe.image = await uploadFile(recipe.image)
    }

    if (imageToDeleteURL) await deleteImage(imageToDeleteURL)
...
export async function removeRecipeById(recipeId) {
  try{
    ...
    if (file.image) await deleteImage(file.image)
...
async function deleteImage(imageURL) {
  try{
    const fileId = getImageIdFromURL(imageURL)
